...
public class DemoteCommand extends CommandBase {
    ...
    public DemoteCommand(KingdomCraftImpl kdc) {
        super(kdc, "demote", 1);
        setArgumentsHint("<player>");
        setExplanationMessage(() -> kdc.getMessages().getMessage("cmdDemoteExplanation"));
        setPermissions("kingdom.demote", "kingdom.demote.other");
    }

    @Override
    public List<String> autocomplete(PlatformPlayer player, String[] args) {
        if ( args.length == 1 ) {
            if ( player.hasPermission("kingdom.demote.other") ) {
            ...
            if ( player.hasPermission("kingdom.demote") && user.getKingdom() != null ) {
        ...
        kdc.getPlugin().getScheduler().executeAsync(() -> {
            try {
                ...
                if ( index == -1 ) {
                    ...
                    kdc.getMessages().send(sender, "cmdDemoteLowest");
                    return;
                } else {
                    rank = ranks.get(index - 1);
                }

                if ( !sender.hasPermission("kingdom.demote.other") ) {
                ...
                if ( !sender.hasPermission("kingdom.demote.other") && rank.getMaxMembers() > 0
