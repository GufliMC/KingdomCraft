...
public class EnemyOtherCommand extends CommandBase {
    ...
    public void execute(PlatformSender sender, String[] args) {
        ...
        if ( existing != null && existing.getType() == RelationType.ENEMY ) {
            kdc.getMessages().send(sender, "cmdEnemyOtherAlready", kingdom.getName(), target.getName());
            return;
        }

        kdc.setRelation(kingdom, target, RelationType.ENEMY);
        kdc.getMessages().send(sender, "cmdEnemyOther", kingdom.getName(), target.getName());
