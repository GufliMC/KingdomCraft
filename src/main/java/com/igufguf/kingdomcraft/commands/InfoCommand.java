...
public class InfoCommand extends CommandBase {
	...
	public InfoCommand(KingdomCraft kingdomCraft) {
		...
		addCallback(new InfoCallback() {
			@Override
			public void onKingdomInfo(InfoBuilder builder, Kingdom kingdom) {
				List<KingdomRelation> relations = kingdomCraft.kingdomHandler.getRelations(kingdom);
			...
			@Override
			public void onPlayerInfo(InfoBuilder builder, Player player) {
	...
	private InfoBuilder build(Kingdom kingdom) {
	...
	private InfoBuilder build(Player player) {
		InfoBuilder builder = new InfoBuilder();
		for ( InfoCallback cb : callbacks ) {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		Player player = kingdomCraft.playerHandler.getPlayer(Bukkit.getPlayer(args[0]));
		if ( player != null ) {
			sender.sendMessage(kingdomCraft.messageHandler.getMessage("cmdInfoPlayer", player.getName()) + "\n" + build(player));
			return;
		}

		Kingdom kingdom = kingdomCraft.kingdomHandler.getKingdom(args[0]);
		if ( kingdom != null ) {
	...
	public interface InfoCallback {
		void onKingdomInfo(InfoBuilder builder, Kingdom kingdom);
		void onPlayerInfo(InfoBuilder builder, Player player);
	}
